  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-4 font-sans">
      <div className="bg-white border border-gray-200 rounded-lg flex flex-col lg:flex-row overflow-hidden shadow-sm">
        ...
        {/* Timer Block */}
        <div className="w-full lg:w-64 flex-shrink-0 p-6 border-b lg:border-b-0 lg:border-r border-gray-200 flex flex-col justify-center">
          <h3 className="text-xl font-bold text-gray-800">Deals and offers</h3>
          <p className="text-gray-500 mb-4">Hygiene equipments</p>
          
          <div className="flex gap-2">
            <div className="bg-gray-800 text-white rounded p-2 text-center w-12">
        ...
        {/* Products Grid */}
        <div className="flex-1 grid grid-cols-2 md:grid-cols-5 divide-x divide-y md:divide-y-0 divide-gray-200">
          {deals.map((deal) => (
            <div key={deal.id} className="p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="h-32 w-32 mb-4 flex items-center justify-center">
                <img src={deal.image} alt={deal.name} className="max-h-full max-w-full object-contain mix-blend-multiply" />
              </div>
              <h4 className="text-gray-800 text-sm mb-2 text-center">{deal.name}</h4>
