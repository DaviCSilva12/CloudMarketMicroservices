      ...
      <div className="bg-white border border-gray-200 rounded-lg flex flex-col lg:flex-row overflow-hidden shadow-sm">
        ...
        <div className="w-full lg:w-64 flex-shrink-0 p-6 relative overflow-hidden h-48 lg:h-auto min-h-[240px]">
          ...
          <div className="relative z-10 w-2/3">
            <h3 className="text-xl font-bold text-gray-900 mb-4 leading-tight">{title}</h3>
            <button className="bg-white text-gray-800 text-sm font-medium px-4 py-2 rounded shadow-sm hover:bg-gray-50 transition-colors">
              {buttonText} &gt;
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 divide-x divide-y divide-gray-200">
          {products.map((product) => (
            <div key={product.id} className="p-4 h-32 flex flex-col justify-between relative cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="z-10">
                <h4 className="text-gray-800 text-sm font-medium line-clamp-1">{product.name}</h4>
                <p className="text-gray-500 text-xs mt-1">From USD {product.price}</p>
              </div>
